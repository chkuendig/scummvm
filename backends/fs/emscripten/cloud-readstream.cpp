...
void CloudReadStream::downloadChunk(uint32 chunkIndex, uint64 chunkStart, uint64 chunkLength) {
	...
	while (downloadRequest->state() == Networking::RequestState::PROCESSING) {
		// Update download progress
		updateDownloadProgress(downloadRequest->getProgress() * chunkLength, chunkLength, downloadStartTime, chunkIndex);
