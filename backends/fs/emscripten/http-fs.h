...
protected:
	 ...
	 */
	HTTPFilesystemNode() : _isDirectory(false), _isValid(false) {}
	 ...
	 */
	void directoryListedCallback(const Networking::JsonResponse &response);
	void requestErrorCallback(const Networking::ErrorResponse &_error);
	void fileDownloadedCallback(const Networking::MemoryWriteStreamDynamicResponse &response);
	void requestFinishedCallback(Networking::Request *invalidRequestPointer);
...
public:
	...
	~HTTPFilesystemNode() {
		warning("HTTPFilesystemNode Destructor %s", _path.c_str());
	}
