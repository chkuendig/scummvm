...
void NetworkReadStreamEmscripten::emscriptenHandleDownload(emscripten_fetch_t *fetch, bool success) {
	// debug(5,"NetworkReadStreamEmscripten::emscriptenHandleDownload %s  status code: %d.", fetch->url, fetch->status);
	...
	if (stream->_request) {
		...
		while (stream->_request && stream->_request->state() != FINISHED && dataRead && !ConnMan.timerStarted()) {
			...
			warning("NetworkReadStreamEmscripten::emscriptenHandleDownload state %d %s", request->state(), fetch->url);
			...
			warning("NetworkReadStreamEmscripten::emscriptenHandleDownload state updated %d %s ", request->state(), fetch->url);
