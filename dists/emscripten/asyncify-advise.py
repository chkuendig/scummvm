...
a_file = open("asyncify-advise.txt")
...
for callee, callers in callees.items() :
    ...
    if(callee.startswith("initial scan") or callee.startswith("operator new(unsigned long)")):
...
print(dict(sorted(filtered_dict.items(), key=lambda item: item[1])))

print(len(filtered_dict.items()))
