...
if [ "$_libmad" = true ]; then
  if [[ ! -f "$LIBS_FOLDER/build/lib/libmad.a" ]]; then
    ...
    # libmad needs patching: https://wiki.linuxfromscratch.org/blfs/wiki/libmad
...
if [[ "run" =~ $(echo ^\(${TASKS}\)$) ]]; then
  ...
  # https://github.com/cloudhead/node-static/issues/241 is menas node-static doesn't work either.
