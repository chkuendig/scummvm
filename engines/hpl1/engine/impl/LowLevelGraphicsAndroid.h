...
//-------------------------------------------------
	...
	public:
		...
		int GetCaps(eGraphicCaps aType) const override;
		...
		Graphics::PixelFormat* GetPixelFormat(){return &mpPixelFormat;}
		...
		void AddVertexToBatch(const cVertex &apVtx) override;
		void AddVertexToBatch(const cVertex *apVtx, const cVector3f* avTransform) override;
		void AddVertexToBatch(const cVertex *apVtx, const cMatrixf* aMtx)override {} ;
		...
		void FlushQuadBatch(tVtxBatchFlag aTypeFlags, bool abAutoClear);
		void ClearBatch();
		...
		//Platform specific
		Bitmap2D* CreateBitmap2DFromData(uint8_t *data,int w, int h, int n,const tString& asType);
		void SetupGL();
