...
namespace hpl
{
	class cAndroidBitmap2D : public Bitmap2D
	{
	public:
		cAndroidBitmap2D(const cVector2l &size, const Graphics::PixelFormat &format);
		//cAndroidBitmap2D(uint8_t *data,int w,int h,int c,const tString& asType);
		~cAndroidBitmap2D();

		bool SaveToFile(const tString& asFile);

		void DrawToBitmap(Bitmap2D *apBmp, const cVector2l &avDestPos);
		bool HasAlpha();
		/**
