...
#include "hpl1/engine/graphics/AndroidBitmap2D.h"
...
namespace hpl
{
	...
	cAndroidBitmap2D::cAndroidBitmap2D(const cVector2l &size, const Graphics::PixelFormat &apPxlFmt) : Bitmap2D(size, apPxlFmt)
	{
		
		create(size, apPxlFmt);
	}

	/*cAndroidBitmap2D::cAndroidBitmap2D(uint8_t *data,int w,int h,int c,const tString& asType)
	 : Bitmap2D("Android",nullptr)
	{
	...
	void cAndroidBitmap2D::DrawToBitmap(Bitmap2D *apDest, const cVector2l &avDestPos)
	{
		...
		for(unsigned int y=0;y<_height;y++)
			for(unsigned int x=0;x<_width;x++)
			{
				...
				unsigned char* srcPix = &srcBuffer[(y*_width + x) * lSrcSize];
	...
	/*void cAndroidBitmap2D::FillRect(const cRect2l &aRect, const cColor &aColor)
	{
		...
		rect.x = aRect.x;
		rect.y = aRect.y;
		rect.w = aRect.w<=0?_width:aRect.w;
		rect.h = aRect.h<=0?_height:aRect.h;
